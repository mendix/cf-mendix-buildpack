...
def set_up_logging_file():
    ...
    if log_ratelimit is None:
        ...
        subprocess.Popen(
            ["./bin/mendix-logfilter", "-r", log_ratelimit, "-f", "log/out.log"]
        )
...
def configure_debugger(m2ee):
    ...
    if debugger_password is None:
        logger.debug(
            "Not configuring debugger, as environment variable " "was not found"
        )
        return
    ...
    if os.getenv("DEPLOY_PASSWORD"):
        ...
        if use_instadeploy(mx_version):
            ...
            thread = instadeploy.InstaDeployThread(
                get_deploy_port(), restart_callback, reload_callback, mx_version
            )
