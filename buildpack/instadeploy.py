...
from buildpack import mxbuild, util
...
class InstaDeployThread(threading.Thread):
    ...
    def __init__(
        self, port, restart_callback, reload_callback, mx_version, java_version
    ):
        super().__init__()
        self.port = port
    ...
    def run(self):
        logging.debug("Going to start mxbuild in serve mode")
        ...
        time.sleep(10)
        logging.debug("Listening on port %d for MPK uploads" % int(self.port))
    ...
    def process_request(self):
        try:
            ...
            if "file" in form:
                ...
                logging.debug(mxbuild_response)
                ...
                if mxbuild_response["status"] != "Success":
                    ...
                    logging.info("Restarting app after MPK push")
                    self.server.restart_callback()
                    state = "STARTED"
                else:
                    logging.info("Reloading model after MPK push")
            ...
            logging.warning(
                "InstaDeploy terminating with MxBuildFailure: {}".format(
                    mbf.mxbuild_response
                )
            )
            return (200, {"state": "FAILED"}, mbf.mxbuild_response)

        except Exception:
            logging.warning("Instadeploy failed", exc_info=True)
            return (
                500,
