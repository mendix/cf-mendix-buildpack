...
from lib.m2ee import M2EE as m2ee_class, logger
...
if os.environ.get("DEBUG_CONTAINER", "false").lower() == "true":
    logger.warning(Maintenance.MESSAGE)
...
def set_up_m2ee_client(vcap_data):
    ...
    m2ee = m2ee_class(
        ...
        config={
            "m2ee": {
                ...
                "admin_pass": security.get_m2ee_password(logger)
            }
        },
    )
    ...
    if os.path.isdir(mendix_runtimes_path) and not os.path.isdir(
        ...
        if process.returncode != 0:
            logger.info("Mendix %s is not available in the rootfs", version)
            logger.info(
    ...
    java_version = runtime.get_java_version(m2ee.config.get_runtime_version())[
        "version"
    ]
    java.update_config(m2ee.config._conf["m2ee"], vcap_data, java_version)
    runtime.set_jetty_config(m2ee)
    newrelic.update_config(m2ee, vcap_data["application_name"])
    appdynamics.update_config(m2ee, vcap_data["application_name"])
    runtime.set_application_name(m2ee, vcap_data["application_name"])
...
def emit(**stats):
    ...
    logger.info("MENDIX-METRICS: %s", json.dumps(stats))
...
if __name__ == "__main__":
    try:
        ...
        logger.error("Starting app container failed: %s", ex)
